$(function(){
	...
	$.ajax({
		...
		success: function(data){
			...
			if(data.result == 0){
				...
				$(".touxiang .right").append('<img id="showImage" src="'+imgstr+'" onerror="this.src='+errorheadimg+';"/>');
				$(".top .name").text(decodeURI(content.nickname));
				$(".personame #personame").val(decodeURI(content.nickname));
	...
	$("#personame").blur(function(){
		var new_value = encodeURI(stripscript($("#personame").val()));
		var new_value2 = stripscript($("#personame").val());
		if(new_value ==""){
