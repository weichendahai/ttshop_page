...
$(function(){
		...
		if(data.result == 0){
				if(addresshtml != null){//判断是不是换了收货地址
					...
					if($(".dizhi").attr("order_addr_id") == -1){//如果没有收货地址
						...
						var addstr = '<div class="lianxiren"><p>'
						+decodeURI(data.content.contact)+
						'</p><span class="phone">'
						+data.content.mobile_phone+
						'</span><span class="moren">默认</span></div><div class="dizhixiangqing"><span class="iconfont">&#xe632;</span>'
				...
				for(var i = 0;i<data.content.items.length;i++){
					...
					orderstr+='</div><div class="bottom"><span class="left">￥'
					+price+'</span><span class="right">x'
				...
				$(".footer p").eq(0).children(".zongji").html("总计：￥"+(priceall/100).toFixed(2));
				$(".footer p").eq(0).children(".zongjiyunfei").html("（运费￥"+(data.content.freight/100).toFixed(2)+"）");
				$(".footer p").eq(0).children(".zongji").attr("money","￥"+(priceall/100).toFixed(2));
				$(".footer p").eq(0).children(".zongji").attr("moneyy",priceall);
	...
	function buynows(data){
		if(data.result == 0){
				...
				for(var i = 0;i<data.content.items.length;i++){
					...
					orderstr+='</div><div class="bottom"><span class="left">￥'
					+price+'</span><span class="right">x1</span></div></div></div>'
	...
	$("#youhui .bottom li").eq(0).click(function(){//清空
		...
		youhuimoney = 0;
		var money_2 = youhuimoney+hongbaomoney;
		$(".footer").find("p").eq(1).text("已优惠："+(money_2).toFixed(2));
	})
	...
	$("#hongbao").delegate(".btn","click",function(){
		if($(this).parents("li").attr("oldclick")==1){
			...
			var b = $(".hongbao .right").html();
			hongbaomoney = parseFloat(b.substring(1,b.length));
			var money_2 = hongbaomoney+youhuimoney;
			$(".footer").find("p").eq(1).text("已优惠："+(money_2).toFixed(2));
		}
	})
	$("#youhui").delegate(".btn","click",function(){

		var moneyy = parseInt($(".zongji").attr("moneyy"));
		...
		if(moneyy >=parameter_first){
			if($(this).parents("li").attr("oldclick")==1){
				...
				$(this).parents("li").attr("oldclick",1);
				var b = $(".youhuiquan .right").html();
				youhuimoney = parameter_second;
				var money_2 = hongbaomoney+youhuimoney;
				$(".footer").find("p").eq(1).text("已优惠："+(money_2).toFixed(2));
			}
		}else{
