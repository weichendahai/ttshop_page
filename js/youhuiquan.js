	...
	function giftcard(page,state){
		...
		$.ajax({
			...
			success: function(data){
				console.log(data)
				if(data.result == 0){
					var content = data.content.coupons;
					console.log(content)
					if(content ==''){
						var str = '<img src ="../img/kong.png" style="display:block;margin:0 auto;width:50%;"/><p style="text-align:center;height:1rem;line-height:1rem;">暂无信息</p>'
						$(".mui-table-view").html(str)
					}else{
						var str ='';
						for(var i = 0;i<content.length;i++){
							var price = (content[i].gift_card_balance/100).toFixed(2)
							str +='<li><div class="ltop"><div class="left"><p>'
								+data.content.coupons[i].conditions[0].parameter_second/100+
								'</p><a>￥</a></div><div class="right"><p>满'
								+data.content.coupons[i].conditions[0].parameter_first/100+
								'元可用</p><p>'
								+decodeURI(content[i].coupon_name)+
								'</p></div></div><div class="bottom">有效期至：'
								+content[i].end_date+
								'</div></li>'
						}
