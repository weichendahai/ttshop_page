...
//	})
	...
	$(".shangpin").on("tap",".jian",function(event){
		...
		var obj = $(this);
		if(shopnum<=2){
			$(this).removeClass("shense").addClass("danse");
		}
		if(shopnum<=1){
	...
	$(".shangpin").on("tap",".jia",function(event){
		shopnum = $(this).parent(".num").children("#num").val();
		$(this).parent(".num").children(".jian").removeClass("danse").addClass("shense");
		cartItemId = $(this).parents("li").attr("cartItemId");
		var obj = $(this);
//		if(shopnum >= 99){
...
//		}
	...
	//全选
	$(".quanxuan .iconfont").click(function(){
	...
	$(".shangpin").on("tap",".iconfont",function(event){
		if($(this).parent("li").hasClass("check")){//如果是选中状态
			$(".quanxuan .iconfont").html("&#xe64a;")
			$(".quanxuan .iconfont").removeClass("checkall");
	...
	function jiage(obj){
		...
		$(".jiesuan .left span").eq(0).text(money.toFixed(2));
	}
				...
				if(data.result == 0){
					...
					if(content ==''){
						...
						for(var i = 0;i<content.items.length;i++){
							...
							if(content.items[i].item_count ==1){
								var jianclass ="danse";
							}else{
								var jianclass ="shense";
							}
							...
							'"><span class="iconfont">&#xe629;</span><img src = "'
							+content.items[i].item_icon+
							'"/><div class="center"><h5>'
							+decodeURI(content.items[i].item_name)+
							'</h5><p class="xinghao">'
							if(content.items[i].item_propertys.length){
								str +='<span >规格:</span>'
							}
							...
							+price+
							'</a></span></p></div><div class="right"><span class="daojishi"></span><div class="num"><span class="jian '+jianclass+'">-</span><input type="number" readonly="readonly" name="num" id="num" value="'
