$(function(){
					...
					if(data.content.items == ''){
						...
						for(var i =0 ;i<data.content.items.length;i++){
							...
							str+='</div><div class="productimg"><img src="'
								...
								'</p><div class="productbottom"><span class="buynum">'
								+data.content.items[i].order_count+
								'人购买</span><span class="iconfont share">&#xe6a2;</span><span class="addShoppingcar iconfont">&#xe724;</span></div></div></div>'
						}
	...
	$(".mui-table-view").on("tap",".addShoppingcar",function(event){//是$(".addcar")这个元素点击促发的 开始动画的位置就是这个元素的位置为起点
		...
		$(".changexinghao .top .topright .title").text(addcar.parents(".box").find(".productname").text());
		$(".changexinghao .top .topimg img").attr("src",img);
		$(".changexinghao .topright .money").text(addcar.parents(".box").find(".money").text());
